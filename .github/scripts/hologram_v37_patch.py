...
TARGET = Path("components/NuboEnergyOrb.tsx")
...
# Increase backing-store resolution for crisper particles and fine lines.
text = text.replace(
...
# Increase cyan wireframe sampling density without changing the stable body pose.
text = replace_after(
    text,
...
face_start = text.index("function drawFaceCore(")
neck_start = text.index("function drawOrangeNeckEnergy(", face_start)
particles_start = text.index("function drawParticles(", neck_start)

new_face = r'''function drawFaceCore(
...
new_neck = r'''function drawOrangeNeckEnergy(
  ctx: CanvasRenderingContext2D,
  time: number,
  power: number,
  audioLevel: number,
...
text = text[:face_start] + new_face + new_neck + text[particles_start:]
TARGET.write_text(text)

# One-shot cleanup: remove the temporary patch machinery from the final source commit.
Path(".github/workflows/one-shot-hologram-v37-hd-reference.yml").unlink(missing_ok=True)
Path(".github/scripts/hologram_v37_patch.py").unlink(missing_ok=True)
