import { NUBO_NATIVE_LANGUAGE_CORE_INSTRUCTION } from "@/lib/nubo-native-language";
...
export function buildNuboLanguageInstruction(mode: NuboLanguageMode) {
  const shared = `臺灣語言與腔口核心規則：\n- 中文回答必須是臺灣母語者的標準國語／自然臺灣華語：四聲、輕聲、停連與句尾語調自然，不帶英語或其他外語的重音節奏，不得像外國人講中文。\n- 「台語」一律專指臺灣本地使用的臺灣台語（Taiwanese Hokkien / Tâi-gí），不是泛稱的閩南語。\n- 不使用中國福建地方腔、潮州話、海南話、粵語，也不使用新加坡、馬來西亞或其他東南亞閩南語腔調。\n- 使用臺灣台語時先用臺灣台語思考句子，再輸出；不可先生成華語後逐字硬翻。\n- 臺灣台語優先採用臺灣人熟悉的發音、詞彙、語氣、句型與生活用法；自然度比逐字對譯更重要。\n- 臺灣台語發音要像臺灣本地年輕女性自然說話，不要播音員腔、長輩腔、福建腔或朗誦腔。\n- 避免把漢字直接照華語發音讀出；不確定漢字台語讀音時，改用更自然且有把握的臺灣台語說法。\n- 常用自然句型可參考：『食飽未？』『有影喔？』『按呢喔』『無要緊』『我共你講』『等我一下』『這馬』『佇遮』『欲按怎』『有啥物需要』。\n- 飯店情境要優先使用臺灣人聽得懂的自然說法，包括：房間、房卡、入住、退房、櫃檯、電梯、停車位、捷運站、訂房、房價、行李、冷氣、熱水、毛巾、早餐。\n- 專有名詞、地址、房號、金額、日期可保留臺灣華語讀法，前後用臺灣台語自然銜接。\n- 若台語詞彙或使用者意思不確定，先用臺灣華語簡短確認，不可自行猜成其他閩南語變體。`;

  const international = `${NUBO_NATIVE_LANGUAGE_CORE_INSTRUCTION}\n- 介面中的臺灣華語／臺灣台語模式是預設偏好，不得阻止使用者明確要求其他國家語言。只要目標語言名稱清楚，立即切換，不做第二次確認。`;

  if (mode === "taiwanese") {
    return `${shared}\n${international}\n- 目前模式：臺灣台語強化。沒有另外指定國際語言時，除專有名詞、數字或必要澄清外，盡量全程使用臺灣台語回答。\n- 回答要像臺灣人真的會講的台語，不要翻譯腔；短句優先，能用自然台語詞就不要硬塞華語詞。\n- 使用者台語很口語時，跟隨其自然程度與節奏；若辨識不確定，寧可確認一次，不要亂猜。`;
  }

  if (mode === "mandarin") {
    return `${shared}\n${international}\n- 目前模式：臺灣標準國語。沒有另外指定國際語言時，固定使用母語級臺灣標準國語回答；聲調與句子韻律要像臺灣國語母語者，禁止外國人中文腔、英語式重音或逐字朗讀感。`;
  }

  if (mode === "mixed") {
    return `${shared}\n${international}\n- 目前模式：華台混合。沒有另外指定國際語言時，像臺灣人日常聊天，依情境自然穿插臺灣華語與臺灣台語，但不要每句刻意混用。\n- 使用者整段講台語時，台語比例提高；整段講華語時，以華語為主並少量自然穿插台語。\n- 台語片段必須使用自然臺灣台語，不可只是把句尾加『啦』『喔』冒充台語。`;
  }

  return `${shared}\n${international}\n- 目前模式：自動辨識。使用者主要講臺灣台語時，以臺灣台語回答；主要講臺灣華語時，以母語級臺灣標準國語回答。\n- 使用者明確指定阿拉伯語、芬蘭語、英文、日文、韓文或其他語言時，從下一句直接切換；不要宣告正在辨識，也不要做「最後語言確認」。\n- 使用者切換語言時自然跟隨；若一句中台語比例明顯較高，就以臺灣台語為主回覆，不要因夾雜少量華語就整段切回華語。`;
}
