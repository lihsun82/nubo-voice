...
const openYouTube = tool({
  name: "open_youtube",
  description: "依歌曲、歌手、影片或主題在Windows開啟YouTube或YouTube Music搜尋。",
  parameters: z.object({
...
2. 使用者想聽音樂或看影片時，確認歌曲／歌手／主題後呼叫open_youtube。
3. 使用者問郵件時，先用gmail_search，再視需要用gmail_read；摘要時不得捏造內容。
4. 使用者要寄信時，可直接建立草稿；若要正式寄出，先呼叫gmail_prepare_send並完整覆誦預覽。只有下一句明確表示「確認寄出」才呼叫gmail_confirm_send。
5. 使用者要求每天或每小時自動整理Gmail時，建立sourceType=gmail的brief任務。
6. 使用者要求把排程結果寄信時，設定deliveryType。gmail_send只有系統白名單允許才會自動寄出，否則會自動改成草稿。
