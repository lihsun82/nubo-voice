...
import {
  ...
  type NuboLiveLatencySnapshot,
} from "@/lib/nubo-live-latency";

function ms(value: number | null) {
  return value === null ? "尚未量到" : `${Math.round(value)} ms`;
}

function formatTime(value: number | null) {
  if (value === null) return "尚未發生";
  return new Date(value).toLocaleTimeString("zh-TW", {
    hour12: false,
  });
}

function buildDiagnosis(snapshot: NuboLiveLatencySnapshot) {
  const notes: string[] = [];
  ...
  if (
    ...
    snapshot.websocketOpenMs >= 1_000
  ) {
    notes.push(
      `手機直接連到Google Gemini WebSocket花了${Math.round(snapshot.websocketOpenMs)}ms，主要瓶頸在手機／Wi-Fi到Google的網路路由。`,
    );
  }

  if (
    snapshot.setupHandshakeMs !== null &&
    snapshot.setupHandshakeMs >= 1_000
  ) {
    notes.push(
  ...
  if (
    ...
    notes.push(
      `手機瀏覽器取得麥克風花了${Math.round(snapshot.microphoneReadyMs)}ms，權限、音訊裝置或省電管理正在拖慢啟動。`,
    );
  }

  if (
    snapshot.transcriptToFirstAudioMs !== null &&
    snapshot.transcriptToFirstAudioMs >= 2_500
  ) {
    if (
      snapshot.toolDurationMs !== null &&
      snapshot.toolDurationMs >= 1_500
    ) {
      notes.push(
  ...
  if (
    ...
    snapshot.toolResponseToFirstAudioMs >= 1_500
  ) {
    notes.push(
  ...
  if (notes.length === 0 && hasTurnMeasurement) {
    notes.push(
      "這次全鏈路數據沒有出現明顯異常；若主觀仍慢，需比較多次測試，確認是否只有特定問題或特定工具較慢。",
    );
  }
...
export function NuboLiveLatencyPanel() {
  const [snapshot, setSnapshot] =
    useState<NuboLiveLatencySnapshot>(() =>
      getNuboLiveLatencySnapshot(),
    );
  ...
  return (
    <details className="nubo-latency-panel" open>
      ...
      <div className="nubo-latency-result">
        ...
        <p>
          麥克風啟動：{ms(snapshot.microphoneReadyMs)}｜按下啟動到可聆聽：
        ...
        <p>
          ...
          {snapshot.audioPacketCount.toLocaleString()}
        </p>
        <p>
        ...
        <p>
          ...
          {formatTime(snapshot.updatedAt || null)}
        </p>
        {snapshot.websocketCloseCode !== null ? (
          <p>
