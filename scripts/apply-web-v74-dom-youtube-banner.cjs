...
patch('components/GeminiVoiceConsole.tsx', (source) => {
  ...
  // Add a React-owned embedded YouTube state. This is the authoritative player surface.
  ...
  // Fast-route commands must also render the same React banner (not just model tool calls).
  ...
  // Replace the V73 native-bridge authoritative route with the React DOM player route.
  ...
  // Replace the old mobile YouTube link with the exact bottom full-width 16:9 banner layout.
