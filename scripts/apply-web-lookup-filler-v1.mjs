...
if (!consoleSource.includes('NUBO_LOOKUP_FILLER_V1')) {
  ...
  const audioPatch = `              if (part?.inlineData?.data) {\n                window.speechSynthesis?.cancel();\n                try {\n                  const nativeBridge = (window as typeof window & {\n                    NuboNative?: { stopLookupFiller?: () => boolean };\n                  }).NuboNative;\n                  nativeBridge?.stopLookupFiller?.();\n                } catch {}\n                noteVoiceInteraction();`;
...
const newSafety = '- 簡單問題直接回答。需要查詢外部或即時資料時，前端會在收到旅客查詢語句的第一時間播放一句自然承接語（例如「好喔，我幫你查一下嘿，稍等我一下唷」）；Android 使用原生台灣中文 TTS 播放，並與後端工具查詢同步進行。你必須立即呼叫正確工具，不等待承接語播完，也不要重複另一句等待語。禁止長篇拖延、禁止假裝已查到結果。';
if (toolsSource.includes(oldSafety)) {
  ...
  const previousSafetyA = '- 簡單問題直接回答。需要查詢外部或即時資料時，前端會在收到旅客查詢語句的第一時間播放一句自然承接語（例如「好喔，我幫你查一下嘿，稍等我一下唷」）；你必須同時立即呼叫正確工具，不等待承接語播完，也不要重複另一句等待語。禁止長篇拖延、禁止假裝已查到結果。';
  const previousSafetyB = '- 簡單問題直接回答。需要查詢外部或即時資料時，前端會先播放一句很短、自然的承接語（例如「嗯，我幫你看一下」）；你必須同時立即呼叫正確工具，不等待承接語播完，也不要重複另一句等待語。禁止長篇拖延、禁止假裝已查到結果。';
  if (toolsSource.includes(previousSafetyA)) {
    toolsSource = toolsSource.replace(previousSafetyA, newSafety);
  } else if (toolsSource.includes(previousSafetyB)) {
    toolsSource = toolsSource.replace(previousSafetyB, newSafety);
  } else {
...
console.log('Applied NUBO lookup filler V1: native Android TTS first + browser fallback + concurrent backend lookup');
