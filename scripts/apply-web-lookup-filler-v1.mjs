...
const newSafety = '- 簡單問題直接回答。需要查詢外部或即時資料時，前端會先播放一句很短、自然的承接語（例如「嗯，我幫你看一下」）；你必須同時立即呼叫正確工具，不等待承接語播完，也不要重複另一句等待語。禁止長篇拖延、禁止假裝已查到結果。';
...
console.log('Applied NUBO lookup filler V1: audible short filler + immediate backend lookup');
