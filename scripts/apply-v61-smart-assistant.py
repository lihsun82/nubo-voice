...
# Preserve the V60 stable Android baseline (YouTube, wake, UI hiding, Google Home 1.10.0).
...
# Persist Google Home permission success. Reopening NUBO no longer forces onboarding every time.
...
old_list_catch = '''            } catch (error: Exception) {
                callback.onResult(errorPayload(error))
            }
        }
    }
...
'''
new_list_catch = '''            } catch (error: Exception) {
                // If the Home SDK can no longer enumerate devices, allow the next explicit
                // setup action to reopen the permission flow instead of remaining stuck.
    ...
    override fun control(
'''
g = replace_once(g, old_list_catch, new_list_catch, "clear stale permission on list failure")

gh.write_text(g)

for token in [
    "versionCode 61",
    '0.61.0-smart-assistant-persistent-home',
]:
    if token not in app.read_text():
        raise SystemExit(f"missing V61 marker: {token}")
